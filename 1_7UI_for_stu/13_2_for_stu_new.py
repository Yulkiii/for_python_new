...
def send_request(account):
    ...
    def send_to_host():
        ...
        time1=time1.split("/")
        time2=time1[1]
        f = open("send\\" + str(time2) + ".txt", "w", encoding="utf-8")
        str1=account+'\n'
        #str1+="0"+"\n"#0 or 1 different teacher
        for i in range(32):
            ...
            if (globals()[name].get() != ""):
                ...
                if (globals()[name].get() == ""):
                    ...
                    str1 += globals()[name].get()+"/"+"0"
                    ws.write(1 + i, 4, label=globals()[name].get())
        wb.save("send\\" + account + ".xls")
        f.write(str1)
        f.close()
        send_mail2('3067842904@qq.com', "send\\" + account + ".xls", subject1='',
                   content='Hellow ,Welcome to use POPFLY,account:'+account+"'s task number:"+time2)
        send_mail("c", "send\\" + str(time2)+ ".txt")
        tkinter.messagebox.askyesno('noting:', "your task number:"+time2)
        send_requestw.quit()
        send_requestw.destroy()
        root.quit()
        root.destroy()
